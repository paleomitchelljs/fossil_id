...
SVG_DIR = os.path.join(HERE, "svgs_beak")

with open(os.path.join(SVG_DIR, "manifest.json")) as f:
    cases = json.load(f)

fig, axes = plt.subplots(len(cases), 3, figsize=(5.5 * 3, 4.0 * len(cases)),
...
for r, c in enumerate(cases):
    for col, v in enumerate(["top", "front", "side"]):
        ...
        ax.set_title(f"{v.upper()}", fontsize=10, color="#6b3410")
    axes[r][0].set_ylabel(c["label"], fontsize=12, fontweight="bold",
                          color="#6b3410", rotation=0, labelpad=130,
                          ha="right", va="center")

out = os.path.join(HERE, "beak_lateral.png")
